...
import { readStorage, writeStorage } from "../shared/local-storage";
...
const PATH_MIME = "text/ember-path";
let treeDragPath = "";
        ...
        part.type === "url" ? (
          <a
            ...
            onClick={(event) => {
              ...
              void window.ember.app.openExternal(part.value);
            }}
          >
            {part.value}
          </a>
...
const INSPECT_WIDTH_KEY = "ember.inspectWidth";
...
function readInspectWidth(): number {
  const raw = Number(readStorage(INSPECT_WIDTH_KEY));
  if (!Number.isFinite(raw)) return INSPECT_DEFAULT;
  return clampInspectWidth(raw);
}

function writeInspectWidth(width: number): void {
  writeStorage(INSPECT_WIDTH_KEY, String(clampInspectWidth(width)));
}

/** Caption buttons for the frameless window on Windows/Linux; macOS keeps its traffic lights. */
function WindowControls() {
  const { t } = useI18n();
  if (window.ember.platform === "darwin") return null;
  return (
    <div className="win-controls">
      <button type="button" aria-label={t("nav.minimize")} onClick={() => void window.ember.window.minimize()}>
        <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
          <path d="M0 5h10" stroke="currentColor" strokeWidth="1" />
        </svg>
      </button>
      <button type="button" aria-label={t("nav.maximize")} onClick={() => void window.ember.window.toggleMaximize()}>
        <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
          <rect x=".5" y=".5" width="9" height="9" fill="none" stroke="currentColor" strokeWidth="1" />
        </svg>
      </button>
      <button type="button" className="close" aria-label={t("nav.closeWindow")} onClick={() => void window.ember.window.close()}>
        <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true">
    ...
    <div className="web-tool">
      ...
      {card.sources.map((source) => (
        <button
          ...
          onClick={() => void window.ember.app.openExternal(source.url)}
        >
...
export function InspectPanel({
  ...
  useEffect(() => window.ember.workspace.onChanged(() => {
  ...
  useEffect(() => {
    ...
    let gone = false;
    void window.ember.workspace.list(workspace).then((next) => {
        ...
        <Fold title={t("inspect.files")} open={working} onToggle={() => setWorking((current) => !current)}>
          <div className="tree">
            <button
              ...
              onContextMenu={(event) => {
                event.preventDefault();
                void window.ember.workspace.reveal(prefix || ".", workspace);
              }}
              onClick={() => {
            ...
            {treeOpen && visible.map((file) => {
              ...
              return (
                <button
                  ...
                  onContextMenu={(event) => {
                    event.preventDefault();
                    void window.ember.workspace.reveal(file, workspace);
                  }}
                >
...
export function FileDrawer({ file, workspace, onClose }: { file: FileChange; workspace?: string; onClose(): void }) {
  ...
  useEffect(() => {
    ...
    void window.ember.workspace.read(file.path, workspace).then(
      (result) => {
  ...
  return (
    <aside className={wide ? "drawer wide" : "drawer"}>
      <header>
        ...
        <button type="button" className="drawer-btn" aria-label={t("preview.open")} onClick={() => void window.ember.workspace.open(file.path, workspace)}>
...
export function ApprovalCard({
  ...
  const respond = async (response: Record<string, unknown>) => {
    try {
      ...
      else await window.ember.agent.respondToUi(request.id, response);
      onDone();
    } catch (error) {
...
export function Login({
  ...
  const modKey = window.ember.platform === "darwin" ? "⌘" : "Ctrl";
  ...
  const listModels = async (target: "chat" | "vision") => {
    ...
    try {
      const ids = await window.ember.auth.listModels(base.trim(), secret.trim());
  ...
  useEffect(() => {
    void Promise.all([
      window.ember.auth.profiles(),
      window.ember.vision.config(),
      window.ember.app.version().catch(() => "0.1.0"),
      ...
      if (url.trim() && key.trim()) {
        void window.ember.auth.listModels(url, key).then(setChatModels).catch(() => undefined);
      }
      ...
      if (vision?.url.trim() && vision.apiKey.trim()) {
        void window.ember.auth.listModels(vision.url, vision.apiKey).then(setVisionModels).catch(() => undefined);
      }
    }).catch(() => undefined);
  }, []);
        ...
        onSubmit={async (event) => {
          ...
          try {
            if (isChatProfileValid(activeCustom)) {
              ...
              await window.ember.auth.saveProfiles({
                kind: "custom",
                deepseek: {
            ...
            if (isVisionProfileValid(activeVision)) {
              await window.ember.vision.saveConfig({
                ...
                <ApiProfilesEditor
                  ...
                  onActiveId={(id) => {
                    ...
                    if (profile?.url.trim() && profile.apiKey.trim()) {
                      void window.ember.auth.listModels(profile.url, profile.apiKey).then(setChatModels).catch(() => undefined);
                    }
                  }}
                  models={chatModels}
              ...
              <>
                ...
                <ApiProfilesEditor
                  ...
                  onActiveId={(id) => {
                    ...
                    if (profile?.url.trim() && profile.apiKey.trim()) {
                      void window.ember.auth.listModels(profile.url, profile.apiKey).then(setVisionModels).catch(() => undefined);
                    }
                  }}
                  models={visionModels}
                      ...
                      {agentSkills.map((skill) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              void window.ember.app.revealPath(skill.name, skill.path).catch((error) => {
              ...
              <div className="about-body">
                <div className="about-hero">
                  ...
                  <button
                    ...
                    onClick={() => void window.ember.app.openExternal("https://github.com/Zhouyeming1/Ember")}
                  >
                    github.com/Zhouyeming1/Ember
                  </button>
                </div>
          ...
          <footer className="settings-foot">
            {configured && pane === "chat" && (
              <button
                ...
                onClick={async () => {
                  await window.ember.auth.logout("deepseek");
                  await onSaved();
                }}
              >
            ...
            {pane === "about" && (
              <>
                <button
                  type="button"
                  className="ghost"
                  onClick={() => void window.ember.app.checkUpdate()}
                >
                ...
                <button
                  type="button"
                  className="ghost"
                  onClick={() => void window.ember.app.openExternal("https://github.com/Zhouyeming1/Ember/issues")}
                >
