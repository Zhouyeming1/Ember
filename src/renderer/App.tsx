...
export function App() {
  ...
  const syncAgentThinking = useCallback(async () => {
    ...
    try {
      const [levelsResp, stateResp] = await Promise.all([
        window.ember.agent.command<{ levels: string[] }>("get_available_thinking_levels"),
        window.ember.agent.command<{ thinkingLevel?: string; model?: { id?: string } }>("get_state"),
      ]);
      ...
      await window.ember.agent.command("set_thinking_level", { level: next }).catch(() => undefined);
    } catch {
  ...
  const applyEffort = useCallback((next: string) => {
    ...
    setEffort(next);
    writeStoredEffort(next);
    void window.ember.agent.command("set_thinking_level", { level: next }).catch(() => undefined);
  }, []);
  ...
  const darwin = window.ember.platform === "darwin";
  ...
  const refreshAgentSkills = useCallback(async () => {
    const loadDisk = () => window.ember.app.listSkills().catch(() => [] as AgentSkillCommand[]);
    if (!agentCwd.current) {
      setAgentSkills(await loadDisk());
      return;
    }
    try {
      const data = await window.ember.agent.command<{
        commands: Array<{
          name: string;
          description?: string;
          source?: string;
  ...
  const refresh = useCallback(async () => {
    const [recent, status, threads] = await Promise.all([
      window.ember.workspace.recent(),
      window.ember.auth.status(),
      window.ember.sessions.list(),
    ]);
  ...
  const resolveSandbox = useCallback(async (asProject: boolean, mode: PermissionMode, cwd?: string) => {
    ...
    if (window.ember.platform === "darwin" || !cwd) return "workspace-write" as const;
  ...
  const startAgent = useCallback(async (
    ...
    try {
      accounts = await window.ember.auth.status();
    } catch (error) {
    ...
    if (seq !== startSeq.current) return false;
    if (asProject && sandbox !== "danger-full-access" && window.ember.platform !== "darwin") {
      setLoading(false);
      setToast(t("toast.sandboxCancelled"));
      return false;
    }
    try {
      const snapshot = await window.ember.agent.start({
        ...(cwd ? { cwd } : {}),
        project: asProject,
        provider: "deepseek",
      ...
      if (modelId) {
        setModel(modelId);
        await window.ember.agent.command("set_model", { provider: "deepseek", modelId }).catch(() => undefined);
      }
      applyThinkingForModel(modelId);
      const nextEffort = effortRef.current;
      await window.ember.agent.command("set_thinking_level", { level: nextEffort }).catch(() => undefined);
      await window.ember.agent.command("set_auto_compaction", { enabled: true }).catch(() => undefined);
      if (seq !== startSeq.current) return false;
      ...
      void window.ember.sessions.list().then(setSessions);
  ...
  const ensureModelReady = useCallback(async (): Promise<boolean> => {
    ...
    if (agentModelIdsRef.current.includes(next)) {
      try {
        await window.ember.agent.command("set_model", { provider: "deepseek", modelId: next });
  ...
  const switchModel = useCallback((next: string) => {
    ...
    if (agentCwd.current && agentModelIdsRef.current.includes(next)) {
      void window.ember.agent.command("set_model", { provider: "deepseek", modelId: next })
  ...
  const bindProject = useCallback(async (cwd: string): Promise<boolean> => {
    ...
    if (!running) await window.ember.agent.stop().catch(() => undefined);
    return true;
  }, [fillPrompt, running, t]);
  const openFolder = useCallback(async () => {
    const selected = await window.ember.workspace.choose();
    if (!selected) return;
    if (!(await bindProject(selected))) return null;
    setWorkspaces(await window.ember.workspace.recent());
    return selected;
  }, [bindProject]);
  ...
  const newThread = useCallback(async () => {
    ...
    if (!agentCwd.current) return;
    agentCwd.current = undefined;
    await window.ember.agent.command("abort").catch(() => undefined);
    await window.ember.agent.stop().catch(() => undefined);
  }, [fillPrompt]);
  ...
  const removeSession = useCallback(async (session: SessionSummary) => {
    ...
    try {
      await window.ember.sessions.remove(session.id);
      setSessions(await window.ember.sessions.list());
    } catch (error) {
  ...
  const pinSession = useCallback(async (session: SessionSummary) => {
    try {
      await window.ember.sessions.pin(session.id, !session.pinned);
      setSessions(await window.ember.sessions.list());
    } catch (error) {
      setToast(error instanceof Error ? error.message : String(error));
    }
  ...
  const renameSession = useCallback(async (session: SessionSummary, title: string) => {
    try {
      await window.ember.sessions.rename(session.id, title);
      setSessions(await window.ember.sessions.list());
    } catch (error) {
      setToast(error instanceof Error ? error.message : String(error));
    }
  ...
  const removeProject = useCallback(async (path: string) => {
    try {
      setWorkspaces(await window.ember.workspace.forget(path));
      setSessions(await window.ember.sessions.list());
    } catch (error) {
      setToast(error instanceof Error ? error.message : String(error));
      return;
  ...
  const applyUndo = useCallback(async (files: RestoreFile[]) => {
    await window.ember.workspace.restore(files, workspace);
    setMessages((current) => dropLastTurn(current));
    const stats = await window.ember.agent.command<{ sessionFile?: string }>("get_session_stats").catch(() => undefined);
  ...
  const stopJobs = useCallback(async (message: string) => {
    const data = await window.ember.agent.command<{ commands: Array<{ name: string }> }>("get_commands");
    ...
    await window.ember.agent.command("prompt", { message });
  }, [t]);
  ...
  const undoLastTurn = useCallback(async () => {
    ...
    try {
      const log = await window.ember.agent.command<{ entries: Parameters<typeof lastTurnRestoreFiles>[0] }>("get_entries");
  ...
  const compactContext = useCallback(async () => {
    ...
    try {
      const result = await window.ember.agent.command<{ tokensBefore?: number; summary?: string }>("compact");
      const [history, nextStats] = await Promise.all([
        window.ember.agent.command<{ messages: unknown[] }>("get_messages"),
        window.ember.agent.command<AgentSessionStats>("get_session_stats"),
      ]);
  ...
  const approvePlan = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await window.ember.agent.command("prompt", { message: "/plan execute" });
  ...
  const refinePlan = useCallback(async (changes: string) => {
    ...
    setLoading(true);
    try {
      await window.ember.agent.command("prompt", {
  ...
  const sendMessage = useCallback(async (preset?: string, images?: string[]) => {
    ...
    if (running) {
      ...
      try {
        ...
        await window.ember.agent.command("steer", payload);
    ...
    try {
      ...
      if (!images?.length) {
        await window.ember.agent.command("prompt", { message: question });
        ...
        try {
          ...
          const message = visionAgentPrompt(question, await window.ember.vision.stage(images));
          await window.ember.agent.command("prompt", { message });
        }
      } else {
        const message = visionAgentPrompt(question, await window.ember.vision.stage(images));
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void window.ember.auth.readApiKey("deepseek").then((key) => {
      if (!key.trim() || !chat.baseUrl) return;
      return window.ember.auth.listModels(chat.baseUrl, key);
    }).then((ids) => {
  ...
  useEffect(() => {
    ...
    const offError = window.ember.agent.onError((message) => {
      if (!live.current) return;
    ...
    const offCommand = window.ember.onAppCommand((command) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void window.ember.workspace.read(".agents/features.json", workspace).then(
        (result) => {
  ...
  const composer = (
    <PromptBar
      ...
      onPermission={(next) => {
        ...
        void (async () => {
          try {
            await window.ember.agent.command("prompt", { message: `/permissions ${mode}` });
    ...
    <div className={["app", darwin && "darwin", fullscreen && "fullscreen"].filter(Boolean).join(" ")}>
            ...
            <div className="messages">
              ...
              {uiRequest && (
                <ApprovalCard
                  ...
                  onRespond={uiRequest.id === "ember:undo" ? async (response) => {
      ...
      {loginOpen && (
        <Login
          ...
          onSaved={async () => {
            const status = await window.ember.auth.status();
            setProviders(status);
            const current = status.find((item) => item.id === "deepseek");
