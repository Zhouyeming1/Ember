...
const userDataPath = path.join(app.getPath("appData"), "Ember");

// Desktop distribution favors a quiet first run; the owner-only file avoids OS keyring prompts.
process.env.EMBER_CREDENTIALS_STORE = "file";
...
function registerIpc(): void {
  ...
  ipcMain.handle("services:reveal-mcp", async () => {
    const file = path.join(getEmberHome(), "mcp.json");
    try {
  ...
  ipcMain.handle("sessions:list", async (_event, cwd?: string) => {
    const threads = await listEmberThreads(cwd ? { cwd } : {});
    return threads.map(
  ...
  ipcMain.handle("auth:status", async (): Promise<ProviderStatus[]> => {
    const credentialStore = await createEmberCredentialStore();
  ...
  ipcMain.handle(
    ...
    async (_event, provider: ApiKeyProviderId) => {
      const stored = await (await createEmberCredentialStore()).read(provider);
  ...
  ipcMain.handle("agent:start", async (_event, options: AgentStartOptions) => {
    ...
    // platform.deepseek.com 是网页控制台、不是 API 域名；误存时改回官方 API，
    // 否则请求会被它的边缘网关以 429 HTML 拦截页挡掉。
    const baseUrl = rawUrl
      ? apiBaseUrl(normalizeDeepSeekRuntimeBaseUrl(rawUrl))
      : undefined;
    // Python 引擎的密钥由主进程读凭据库、经子进程 env 注入（不经过渲染进程）。
    // v1 引擎只实现 DeepSeek/OpenAI 兼容的 ChatLLM，其它 provider 留空 key 会得到友好报错。
    let apiKey: string | undefined;
    if (startOptions.provider === "deepseek") {
      const storedKey = await deepSeekApiKeyFromStore();
...
async function readHomeJson(name: string): Promise<unknown> {
  try {
    return JSON.parse(await fsp.readFile(path.join(getEmberHome(), name), "utf8"));
  } catch {
    return {};
  }
}
...
async function writeHomeJson(name: string, value: unknown): Promise<void> {
  const file = path.join(getEmberHome(), name);
...
async function saveDefaultModel(
  ...
  const settingsPath = path.join(getEmberHome(), "settings.json");
...
async function loadChatProfiles(): Promise<ChatProfiles> {
  ...
  const stored = await (await createEmberCredentialStore()).read("deepseek");
  const apiKey =
...
/** Python worker 的可执行文件：优先 EMBERPY_PYTHON，缺省按平台取 python / python3。 */
...
/** 定位含 emberpy 包的 runtime-py 目录（给子进程做 PYTHONPATH）。 */
function findEmberpyDir(): string {
  const configured = process.env.EMBERPY_DIR?.trim();
  if (configured) return path.resolve(configured);
  // 编译产物在 dist-electron/main，上溯两级回到仓库根；dev 下再用 app 根兜底。
  const candidates = [
    path.resolve(currentDirectory, "../..", "runtime-py"),
    path.join(app.getAppPath(), "runtime-py"),
  ];
  for (const dir of candidates) {
    try {
      if (fs.existsSync(path.join(dir, "emberpy", "rpc.py"))) return dir;
    } catch {
      // 非法路径直接跳过，继续试下一个候选
    }
  }
...
/** platform.deepseek.com 是网页控制台，不是 API 域名；误存时改回官方 API，其它原样透传。 */
function normalizeDeepSeekRuntimeBaseUrl(baseUrl: string): string {
  try {
    return new URL(baseUrl).hostname === "platform.deepseek.com"
      ? "https://api.deepseek.com"
      : baseUrl;
  } catch {
    return baseUrl; // 非法 URL 交给 apiBaseUrl 一并处理
  }
}

/** DeepSeek key：优先凭据库（当前启用 profile 覆盖该槽位），再退回环境变量。 */
async function deepSeekApiKeyFromStore(): Promise<string> {
  const stored = await (await createEmberCredentialStore()).read("deepseek");
  const storedKey =
    stored && stored.type === "api_key" && typeof stored.key === "string"
      ? stored.key.trim()
      : "";
  return storedKey || process.env.DEEPSEEK_API_KEY?.trim() || "";
}
...
async function materializeDeepSeekVision(
  ...
  const store = await createEmberCredentialStore();
  try {
    const profiles = await loadChatProfiles().catch(() => undefined);
