---
name: ember-ui
description: Applies Ember's paper + ink visual language (tokens, cards, chips, buttons, composer, approvals) to the desktop workbench. Use when adding or restyling UI, CSS, or cloning this look.
---

# Ember UI

浅色纸面 + 墨色字。桌面工作台共用这一套，不要做成 IDE、深色终端或 Discord。

先读 [tokens.md](tokens.md) 里的色板和组件，再动最少 CSS。新界面必须看起来像同一张纸上多了一块。

## 配色（先抄这里）

产品的**纸和墨必须一致**。强调色就是墨，不要另起蓝/紫。
...
品牌强调可用皮革棕 `#8a6f5a`（`--brand`）。**桌面壳不要用它当按钮色**，桌面强调仍是墨。

桌面 CSS 变量：`--page` `--surface` `--ink` `--accent`（= ink）。不要另起一套 hex。
...
- 工作台正文 `14px / 1.5`，tracking `-0.011em`。
...
| Logo | 沿用仓库内 `build/` 的 Ember 资产，不做第三方产品的 logo |

工作台壳（侧栏 252、对话贴底、权限四档）约束整个桌面工作台。

克隆同气质产品：抄色板和上表组件，不要抄 Ember 商标与 logo 资产。

## 桌面改 UI 时

`src/renderer/styles.css`、`ui.tsx`、`App.tsx`、`src/shared/i18n.ts`。
