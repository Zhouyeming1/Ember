...
> 面向仓库的 AI 编程桌面工作台。本文面向在 Ember 仓库内做开发的工程师：解释架构、模块职责、关键数据流与常见开发任务。产品说明与用户文档见根目录 `README.md` / `README.zh-CN.md`。

## 1. 项目概览

Ember 是一个基于 Electron 的本地优先 AI 编程桌面工作台：模型调用、工作区工具、终端命令、权限确认、会话历史与 diff 审查集中在一个桌面应用里。UI 与会话数据留在本机，模型请求直连用户配置的 provider 或本地网关。

**核心定位**：Ember 完全自包含，不依赖第三方 agent 运行时。agent 循环、权限/沙箱、工作区工具、文件补丁、checkpoint、hooks、skills、memory 与会话文件都在本仓库实现：桌面主进程用 `src/main/ember-core.ts` 做本地数据层，agent 引擎是 Python 包 `runtime-py/emberpy`（以 `python -m emberpy.rpc` 子进程运行）。Electron 壳负责：

- Electron 壳（窗口、菜单、IPC、权限入口、工作区文件访问、更新检查）
- 渲染进程 UI（React，中文/英文界面）
- 把 `emberpy`（Python RPC worker）桥接成桌面可用的 agent（见 §5）
...
| Vitest 3                  | 单元测试                                           |
| emberpy（Python，runtime-py/）| agent 引擎（RPC worker、权限/沙箱、工具、会话、checkpoint） |
...
Electron Main（src/main）
  ...
  （本地数据层 src/main/ember-core.ts：~/.ember、auth.json、settings、SQLite 会话索引）
        │  换行分隔 JSON-RPC over stdio
        ▼
emberpy —— Python agent 引擎（runtime-py/emberpy，python -m emberpy.rpc 子进程）
  agent 循环 · 权限/沙箱 · 工作区工具 · 补丁与 /undo checkpoint · hooks · skills · memory · 会话文件
```

渲染进程没有任何 Node.js 直连能力；桌面能力全部通过 `src/shared/types.ts` 里定义的类型化 `DesktopApi` 契约走 IPC。agent 运行在独立的 Python 子进程（`python -m emberpy.rpc`），崩溃后磁盘上的会话仍可继续作为对话恢复，但引擎不会静默重放未完成的命令。
...
├── .agents/skills/            # 项目内置技能（init-long-run / continue-long-run / plan-then-act / ember-ui）
├── .github/workflows/release.yml
├── runtime-py/                # Python agent 引擎（emberpy 包 + pytest 测试）
│   └── emberpy/               # rpc 入口、bridge/worker、tools、patches、skills、hooks、memory、session
└── src/
    ├── main/                  # Electron 主进程
    │   ├── index.ts           # 窗口、IPC、工作区、权限、更新、预览协议（约 850 行）
    │   ├── ember-core.ts      # 本地数据层替身：~/.ember home、凭据、会话索引、旧数据迁移
    │   ├── agent-host.ts      # Agent（python -m emberpy.rpc）子进程宿主（spawn / 请求 / 事件分发）
    │   ├── rpc-lines.ts       # 按行切分 RPC stdout（UTF-8 安全）
    │   ├── skills-fs.ts       # 本地技能目录扫描与 reveal
    │   └── update-check.ts    # GitHub Release 更新检查
    ├── preload/
    │   └── index.ts           # contextBridge 暴露 window.ember（DesktopApi）
    ├── renderer/              # React 渲染进程
    ...
    └── extensions/
        └── vision.ts          # 遗留构建；引擎现已内置 `vision` 工具
```
...
- `src/renderer/**` 通过 `window.ember`（类型为 `DesktopApi`）访问桌面能力，不直接 import Electron。
...
- Python `>= 3.11`，且能 import `emberpy`（先 `pip install -e ./runtime-py`）
...
### 3.3 与引擎（emberpy）同时开发

引擎是 Python 包 `runtime-py/emberpy`，源码就在本仓库。改完引擎后 `pip install -e ./runtime-py` 重装即可。桌面主进程定位引擎解释器用 `EMBERPY_PYTHON`（缺省按平台取 `python` / `python3`），`EMBERPY_DIR` 指向含 `emberpy` 的目录作为子进程 `PYTHONPATH`（缺省自动找仓库根下 `runtime-py`）。

## 4. IPC 契约（src/shared/types.ts）

`DesktopApi` 是渲染进程能访问的全部能力的类型契约，`window.ember` 即其实例。所有通道名以 `域:动作` 命名，`invoke`（请求-响应）为主，个别为 `send`（事件）。
...
agent 运行在独立的 Python 子进程里（`python -m emberpy.rpc`），通信协议是**换行分隔的 JSON-RPC over stdio**：
...
- `start()`：先 `stop()` 旧的 → spawn `python -m emberpy.rpc`（解释器：`EMBERPY_PYTHON` 或按平台 `python`/`python3`；`PYTHONPATH` 前置 `EMBERPY_DIR` 或仓库根 `runtime-py`；env 设 `PYTHONUNBUFFERED` / `PYTHONUTF8` / `PYTHONIOENCODING`）→ 返回 `snapshot()`。
...
- DeepSeek 场景把凭据库里的 key 与规范化后的 Base URL 经子进程 env 注入（`DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL`），不经过渲染进程。
- 图片先经 `vision:stage` 落盘到 `userData/uploads`，再以带 `[vision]` 的指示文本发给引擎（见 §9）。
...
- `startAgent(cwd, sessionPath, asProject, resume, mode, seedMessage)` —— 统一入口：校验凭据 → 解析沙箱 → `window.ember.agent.start` → hydrate 消息 → set_model / set_thinking_level 同步 → 刷新会话列表与技能。首条消息时把乐观用户消息作为 `seedMessage` 传入。
...
样式全部在 `styles.css`（手写 CSS，无 UI 框架）。改 UI 气质前先读 `.agents/skills/ember-ui/SKILL.md` 与 `tokens.md`（界面气质规范）。
...
- 语言解析：`resolveLocale` 优先存储值（`~/.ember/settings.json` 的 `locale`），否则跟随系统语言列表；默认 `zh`。

## 7. 会话、恢复与编辑安全

### 7.1 会话文件

会话落盘为 JSONL 文件（`~/.ember/sessions/YYYY/MM/DD/*.jsonl`，另保留平铺硬链给引擎 `--session` 用），由 `src/main/ember-core.ts` 管理（`EmberStateStore` / `listEmberThreads`；`~/.ember/state.sqlite` 只是索引缓存，JSONL 仍是真源），主进程只做映射：`sessions:list` 把 thread 转成 `SessionSummary`（含 `path` 会话文件路径、`storagePath` 分区路径、`preview` 等）。重命名通过**追加**一条 `session_info` 条目并重建索引完成（`sessions:rename`）。
...
引擎（`runtime-py/emberpy/patches`）在文件写入时落 `ember-checkpoint` 条目（含每个文件的 `before` 内容）。`/undo` 流程：

1. `get_entries` 拉会话条目，`lastTurnRestoreFiles` 解析**最后一个真实用户轮之后**、未被 `ember-checkpoint-undone` 撤销过的最新 checkpoint 的 before 文件集。
2. 弹 `ApprovalCard` 确认（`ember:undo` 特殊 id）。
...
后台/进行中的 shell 命令由 `sessionTerminals` 从工具活动里提炼（`process_id` 追踪），在 InspectPanel 展示。当前引擎不提供停 job 命令：需要停止时请重开会话再试（渲染端的提示文案亦如此）。
...
图片识别是 Python 引擎内置工具（`runtime-py/emberpy/tools/vision.py`，工具名固定 `vision`，renderer 按字面量识别并渲染成“识图”轨迹）：

- 前端把粘贴的图片 base64 经 `vision:stage` 落盘到 `userData/uploads`（限制 4 张），再把消息替换成带 `[vision]`（含图片绝对路径）的指示文本发给引擎；会话文件只存暂存路径。
- `vision` 工具只读白名单目录（上传目录与工作区）里的图片 → data URI → OpenAI chat.completions 兼容请求。
- 配置读 `vision-config.json`（默认 `appData/Ember/vision-config.json`）：主路径为 DeepSeek 官方 key 或自定义（GLM 需 `ZHIPU_API_KEY`）；无 key 时返回清晰的引导错误，引擎未内置免 key 的 MinerU OCR。
- 只有用户**本轮**消息里确实贴了图片时工具才可用，避免模型为读图随意调用。

shared 层 `vision-api.ts` 仍提供纯函数（请求构造、`visibleUserText` 等渲染端辅助）。

## 10. 数据与存储布局

| 位置                                                                        | 内容                                                                                                                                   |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `appData/Ember`（userData） | `recent-workspaces.json`（最近工作区）、`vision-config.json`、`chat-profiles.json`、`uploads/`（暂存图片）、`tasks/`（无项目时的 cwd） |
| `~/.ember`（getEmberHome，ember-core.ts 管理；`EMBER_HOME` 可覆盖）                        | `settings.json`（locale / 默认 provider+model）、`sessions/`（JSONL 会话 + `YYYY/MM/DD` 分区）、`archived_sessions/`、`state.sqlite` 索引缓存（`EMBER_SQLITE_HOME` 覆盖目录）、`auth.json` 文件凭据    |
| 项目内 `.agents/`                                                           | `features.json`（跨会话任务清单）、`progress.md`（进度）——由技能约定维护                                                               |

环境变量约定：`EMBER_CREDENTIALS_STORE=file`（index.ts 启动即设，分发版避免钥匙串弹窗）；`EMBER_HOME` 覆盖数据 home，`EMBER_SESSIONS_DIR` / `EMBER_ARCHIVED_SESSIONS_DIR` / `EMBER_SQLITE_HOME` 覆盖对应目录；`EMBERPY_PYTHON` 指定引擎解释器、`EMBERPY_DIR` 指向含 `emberpy` 的目录。

## 11. 技能（Skills）系统

技能由 Python 引擎加载（Ember 不另写 loader），本仓库只做 UI 与文件系统辅助：

- **扫描**：`src/main/skills-fs.ts` 扫用户级（`~/.ember/skills`、`~/.agents/skills`）与项目级（`.agents/skills`，需信任项目）；`app:list-skills` 供设置页展示。
...
- **项目内技能**：`.agents/skills/` 下有 `init-long-run` / `continue-long-run` / `plan-then-act` / `ember-ui`（界面气质）。技能文件必须含 frontmatter `name` + `description`，缺项不加载。
...
| 扩展（遗留） | `src/extensions/vision.ts` | ESM（`.js`）  | `dist-electron/extensions/vision.js`                       |

`electron` 标记为 external（引擎在 Python 侧，不参与 tsup 构建）。
...
- `asar: false`：让 RPC 子进程与外部资源以真实文件路径可见（无 keyring，凭据走 `~/.ember/auth.json` 文件）。
...
postinstall / predev 执行：检查 Electron 发行二进制是否完整，缺失时跑 `install.js`，macOS 下还会从 `~/Library/Caches/electron` 缓存 zip 解压，并把 `Info.plist` 的 `CFBundleName` / `CFBundleDisplayName` 改为 Ember。
...
- 先读 `.agents/skills/ember-ui/SKILL.md` 与 `tokens.md`，保持配色、圆角、间距令牌一致。
- 视觉验收：按仓库约定**不要用 Chrome/headless/截图**做验收（除非用户本轮明确要求截图），写完 HTML/CSS 即可。

### 14.4 加一个 agent 工具或 RPC 命令

1. 命令：加入 `ALLOWED_AGENT_COMMANDS` 白名单；`agent:command` 入参透传给引擎（`emberpy.rpc` worker）。
...
先看引擎（`runtime-py/emberpy`）的会话条目类型（`SessionEntryLike`：`message` / `custom` 条目）；`conversation.ts` 的解析函数与 `lastTurnRestoreFiles` 依赖 `ember-checkpoint` / `ember-checkpoint-undone` 的 customType 约定，改动需两端同步。
...
| /undo 无效果          | 该轮没有写入类工具；或 checkpoint 已被撤销；`get_entries` 里应有 `ember-checkpoint` 条目        |
