...
> A repo-oriented AI coding desktop workbench. This guide is for engineers working inside the Ember repository: architecture, module responsibilities, key data flows, and common development tasks. Product description and user documentation live in `README.md` / `README.zh-CN.md`.

## 1. Project Overview

Ember is a local-first AI coding desktop workbench built on Electron: model calls, workspace tools, terminal commands, permission prompts, session history, and diff review all live in one desktop app. The UI and session data stay on your machine; model requests go directly to the provider or local gateway you configure.

**Core positioning**: Ember is self-contained and depends on no third-party agent runtime. The agent loop, permissions/sandbox, workspace tools, file patches, checkpoints, hooks, skills, memory, and session files are all implemented in this repository — the desktop main process uses `src/main/ember-core.ts` as its local data layer, and the agent engine is the Python package `runtime-py/emberpy` (spawned as `python -m emberpy.rpc`). The Electron shell is responsible for:
...
- Bridging the `emberpy` (Python RPC worker) child process into a desktop-usable agent (see §5)
...
| Vitest 3 | Unit tests |
| emberpy (Python, runtime-py/) | Agent engine (RPC worker, permissions/sandbox, tools, sessions, checkpoints) |
...
React Renderer (src/renderer)
  conversation rendering, diff panel, settings, project/session navigation, composer
        │  contextBridge (window.ember) / Electron IPC
        ▼
Electron Main (src/main)
  windows, workspace files, credentials, AgentHost child-process host, update checks, preview protocol
  (local data layer src/main/ember-core.ts: ~/.ember, auth.json, settings, SQLite session index)
        │  newline-delimited JSON-RPC over stdio
        ▼
emberpy — the Python agent engine (runtime-py/emberpy, spawned as python -m emberpy.rpc)
  agent loop · permissions/sandbox · workspace tools · patches & /undo checkpoints · hooks · skills · memory · session files
```

The renderer has no direct Node.js access; all desktop capabilities cross the typed `DesktopApi` contract defined in `src/shared/types.ts` over IPC. The agent runs in a separate Python child process (`python -m emberpy.rpc`). After a crash, an on-disk session can still continue as a conversation, but the engine never silently replays unfinished commands.
...
├── .agents/skills/            # built-in project skills (init-long-run / continue-long-run / plan-then-act / ember-ui)
├── .github/workflows/release.yml
├── runtime-py/                # Python agent engine (emberpy package + pytest tests)
│   └── emberpy/               # rpc entry, bridge/worker, tools, patches, skills, hooks, memory, session
└── src/
    ├── main/                  # Electron main process
    │   ├── index.ts           # windows, IPC, workspace, permissions, updates, preview protocol (~850 lines)
    │   ├── ember-core.ts      # local data-layer replacement: ~/.ember home, credentials, session index, legacy migration
    │   ├── agent-host.ts      # Agent (python -m emberpy.rpc) child-process host (spawn / requests / event dispatch)
    ...
    ├── preload/
    │   └── index.ts           # contextBridge exposing window.ember (DesktopApi)
    ...
    └── extensions/
        └── vision.ts          # legacy build; the engine now ships the built-in `vision` tool
```
...
- `src/renderer/**` accesses desktop capabilities through `window.ember` (typed as `DesktopApi`); it never imports Electron directly.
...
- Python `>= 3.11` that can import `emberpy` (run `pip install -e ./runtime-py` first)
...
### 3.3 Developing Alongside the Engine (emberpy)

The engine is the Python package `runtime-py/emberpy`, whose source lives in this repository. After changing it, reinstall with `pip install -e ./runtime-py`. The desktop main process picks the engine interpreter via `EMBERPY_PYTHON` (defaults to `python` on Windows and `python3` elsewhere) and points the child's `PYTHONPATH` at `EMBERPY_DIR` (defaults to `runtime-py` under the repo root).

## 4. IPC Contract (src/shared/types.ts)

`DesktopApi` is the type contract for every capability the renderer can access; `window.ember` is its instance. Channels are named `domain:action`; most are `invoke` (request/response), a few are `send` (events).
...
The agent runs in a separate Python child process (spawned as `python -m emberpy.rpc`). The wire protocol is **newline-delimited JSON-RPC over stdio**:
...
- `start()`: `stop()` the old one first → spawn `python -m emberpy.rpc` (interpreter: `EMBERPY_PYTHON` or `python`/`python3` by platform; `PYTHONPATH` is prefixed with `EMBERPY_DIR` or the repo-root `runtime-py`; env sets `PYTHONUNBUFFERED` / `PYTHONUTF8` / `PYTHONIOENCODING`) → return `snapshot()`.
...
- For DeepSeek, the stored API key and a normalized Base URL are injected into the child env (`DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL`), never passing through the renderer.
- Pasted images are staged to `userData/uploads` via `vision:stage` first, then sent to the engine as a `[vision]` indicator message (see §9).
...
- `startAgent(cwd, sessionPath, asProject, resume, mode, seedMessage)` — unified entry: validate credentials → resolve sandbox → `window.ember.agent.start` → hydrate messages → sync set_model / set_thinking_level → refresh session list and skills. On first send, the optimistic user message is passed in as `seedMessage`.
...
All styling lives in `styles.css` (hand-written CSS, no UI framework). Before changing UI look-and-feel, read `.agents/skills/ember-ui/SKILL.md` and `tokens.md` (UI design-token conventions).
...
- Locale resolution: `resolveLocale` prefers the stored value (`locale` in `~/.ember/settings.json`), otherwise follows the system language list; default is `zh`.

## 7. Sessions, Recovery & Edit Safety

### 7.1 Session Files

Sessions are stored as JSONL files (`~/.ember/sessions/YYYY/MM/DD/*.jsonl`, with a flat hard-link kept for the engine's `--session`), managed by `src/main/ember-core.ts` (`EmberStateStore` / `listEmberThreads`; `~/.ember/state.sqlite` is just an index cache — the JSONL is the source of truth). The main process only maps them: `sessions:list` converts threads to `SessionSummary` (including `path` session-file path, `storagePath` partitioned path, `preview`, etc.). Renaming **appends** a `session_info` entry and rebuilds the index (`sessions:rename`).
...
The engine (`runtime-py/emberpy/patches`) records an `ember-checkpoint` entry (with each file's `before` content) on file writes. The `/undo` flow:

1. `get_entries` pulls session entries; `lastTurnRestoreFiles` resolves the `before` file set of the newest checkpoint **after the last real user turn** that has not been undone by `ember-checkpoint-undone`.
2. Show an `ApprovalCard` confirmation (special id `ember:undo`).
...
Background/in-flight shell commands are extracted from tool activity by `sessionTerminals` (tracked by `process_id`) and shown in the InspectPanel. The current engine does not provide a stop-job command: to stop one, start a new thread instead (the renderer copy says the same).
...
## 9. Vision Capabilities (image input)

Image understanding is a built-in tool of the Python engine (`runtime-py/emberpy/tools/vision.py`; tool name is fixed as `vision`, which the renderer recognizes literally and renders as an "image recognition" trace):

- The renderer stages pasted images as base64 into `userData/uploads` via `vision:stage` (max 4), then replaces the message with a `[vision]` indicator text (containing the absolute image paths) sent to the engine; the session file only stores the staged paths.
- The `vision` tool reads images from whitelisted roots (the uploads dir and the workspace) → data URI → an OpenAI chat.completions-compatible request.
- Configuration is read from `vision-config.json` (default `appData/Ember/vision-config.json`): the main path is the official DeepSeek key or a custom provider (GLM needs `ZHIPU_API_KEY`); with no key it returns a clear guidance error, and the engine does not bundle a key-less MinerU OCR.
- The tool is available only when the **current turn's** user message actually pasted images, so the model cannot call it just to look at files.

The shared layer `vision-api.ts` still provides pure helpers (request construction, `visibleUserText`, and other renderer-side utilities).

## 10. Data & Storage Layout

| Location | Contents |
| --- | --- |
| `appData/Ember` (userData) | `recent-workspaces.json`, `vision-config.json`, `chat-profiles.json`, `uploads/` (staged images), `tasks/` (cwd without a project) |
| `~/.ember` (getEmberHome, managed by ember-core.ts; `EMBER_HOME` overrides) | `settings.json` (locale / default provider+model), `sessions/` (JSONL sessions + `YYYY/MM/DD` partitions), `archived_sessions/`, `state.sqlite` index cache (`EMBER_SQLITE_HOME` overrides its dir), `auth.json` file-backed credentials |
| project `.agents/` | `features.json` (cross-session task list), `progress.md` (progress) — maintained by skill conventions |

Environment variables: `EMBER_CREDENTIALS_STORE=file` (set at the top of `index.ts`; avoids keyring prompts in the distributed build); `EMBER_HOME` overrides the data home, `EMBER_SESSIONS_DIR` / `EMBER_ARCHIVED_SESSIONS_DIR` / `EMBER_SQLITE_HOME` override the corresponding directories; `EMBERPY_PYTHON` selects the engine interpreter and `EMBERPY_DIR` points at the directory containing `emberpy`.

## 11. Skills System

Skills are loaded by the Python engine (Ember does not ship a separate loader); this repository only provides UI and filesystem helpers:

- **Scanning**: `src/main/skills-fs.ts` scans user-level (`~/.ember/skills`, `~/.agents/skills`) and project-level (`.agents/skills`, requires trusting the project) roots; `app:list-skills` feeds the settings page.
...
- **Built-in project skills**: `.agents/skills/` contains `init-long-run` / `continue-long-run` / `plan-then-act` / `ember-ui` (UI look-and-feel). Skill files must include frontmatter `name` + `description`; missing either means the skill is not loaded.
...
| Extension (legacy) | `src/extensions/vision.ts` | ESM (`.js`) | `dist-electron/extensions/vision.js` |

`electron` is marked external (the engine lives on the Python side and does not participate in the tsup build).
...
- `asar: false`: the RPC child process and external resources need real filesystem paths (no keyring; credentials live in the `~/.ember/auth.json` file).
...
Runs on postinstall / predev: checks whether the Electron distribution binaries are complete, runs `install.js` when missing, unpacks from the `~/Library/Caches/electron` zip cache on macOS, and patches `CFBundleName` / `CFBundleDisplayName` in `Info.plist` to Ember.
...
- First read `.agents/skills/ember-ui/SKILL.md` and `tokens.md` to keep color, radius, and spacing tokens consistent.
...
1. Commands: add to the `ALLOWED_AGENT_COMMANDS` whitelist; `agent:command` arguments pass through to the engine (`emberpy.rpc` worker).
...
First look at the engine's (`runtime-py/emberpy`) session entry types (`SessionEntryLike`: `message` / `custom` entries); the parsing functions in `conversation.ts` and `lastTurnRestoreFiles` depend on the `ember-checkpoint` / `ember-checkpoint-undone` customType conventions — keep them in sync across both sides.
...
| /undo has no effect | The turn made no write-type tool calls; or the checkpoint was already undone; `get_entries` should show an `ember-checkpoint` entry |
